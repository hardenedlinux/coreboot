...
static int sd_cmd(volatile struct spi_ctrl *spi, uint8_t cmd, uint32_t arg,
		uint8_t crc)
{
	unsigned long n;
	uint8_t r;
...
static void sd_poweron(
		volatile struct spi_ctrl *spi,
		unsigned int input_clk_khz)
{
...
int sd_init(volatile struct spi_ctrl *spi, unsigned int input_clk_khz)
{
...
int sd_copy(volatile struct spi_ctrl *spi,
		void *dst,
		uint32_t src_lba,
		size_t size)
{
